...
namespace SCHALE.GameServer.Utils
{
    ...
    {
        public static Exception NoConnectionStringException = new ArgumentNullException(
            $"ConnectionString in appsettings is missing"
        );
        ...
        public static void AddDbProvider(this IServiceCollection services, IConfiguration conf)
        {
            ...
            {
                case "SQLite3":
                    services.AddDbContext<SCHALEContext, SCHALESqliteContext>(
                        opt =>
                            opt.UseSqlite(
                                    conf.GetConnectionString("SQLite3")
