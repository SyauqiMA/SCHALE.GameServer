...
namespace SCHALE.GameServer.Controllers.Api.ProtocolHandlers
{
    public class Raid : ProtocolHandlerBase
    {
        ...
        public ResponsePacket LobbyHandler(RaidLobbyRequest req)
        {
            ...
            var raidSeasonExcel = excelTableService
                .GetTable<RaidSeasonManageExcelTable>()
                .UnPack()
                .DataList;
            var targetSeason = raidSeasonExcel.FirstOrDefault(x =>
                x.SeasonId == account.RaidInfo.SeasonId
            );
        ...
        public ResponsePacket CreateBattleHandler(RaidCreateBattleRequest req)
        {
            ...
            var raidStageExcel = excelTableService
                .GetTable<RaidStageExcelTable>()
                .UnPack()
                .DataList;
            var bossStatsExcel = excelTableService
                .GetTable<CharacterStatExcelTable>()
                .UnPack()
                .DataList;
            var currentRaidExcel = raidStageExcel.First(x => x.Id == req.RaidUniqueId);
            var currentBossExcel = bossStatsExcel.First(x =>
                x.CharacterId == currentRaidExcel.RaidCharacterId
            );
        ...
        public ResponsePacket EnterBattleHandler(RaidEnterBattleRequest req)
        {
            ...
            var currentTeam = account.Echelons.First(x =>
                x.EchelonType == EchelonType.Raid && x.EchelonNumber == req.EchelonId
            );
        ...
        public ResponsePacket EndBattleHandler(RaidEndBattleRequest req)
        {
            ...
            var raidStageTable = excelTableService
                .GetTable<RaidStageExcelTable>()
                .UnPack()
                .DataList;
            var currentRaidData = raidStageTable.FirstOrDefault(x =>
                x.Id == account.RaidInfo.CurrentRaidUniqueId
            );

            var timeScore = RaidManager.CalculateTimeScore(
                req.Summary.ElapsedRealtime,
                account.RaidInfo.CurrentDifficulty
            );
            ...
            account.RaidInfo.BestRankingPoint =
                rankingPoint > account.RaidInfo.BestRankingPoint
                    ? rankingPoint
                    : account.RaidInfo.BestRankingPoint;
            ...
            bool raidDone = RaidManager.Instance.EndBattle(
                account,
                req.Summary.RaidSummary.RaidBossResults
            );
        ...
        {
            return new RaidOpponentListResponse() { OpponentUserDBs = [] };
        }
    }
}
