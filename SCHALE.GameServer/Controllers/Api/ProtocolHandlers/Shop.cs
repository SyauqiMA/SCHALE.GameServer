...
namespace SCHALE.GameServer.Controllers.Api.ProtocolHandlers
{
    public class Shop : ProtocolHandlerBase
    {
        ...
        public Shop(
            ...
            SharedDataCacheService sharedData,
            ILogger<Shop> logger
        )
        ...
        public ResponsePacket BeforehandGachaPickHandler(ShopBeforehandGachaPickRequest req)
        {
            ...
            {
                GachaResults.Add(
                    new GachaResult(charId) // hardcode until table
                    {
                        Character = new()
                        {
            ...
            return new ShopBeforehandGachaPickResponse() { GachaResults = GachaResults };
        }
        ...
        public ResponsePacket ShopBuyGacha3ResponseHandler(ShopBuyGacha3Request req)
        {
            ...
            for (int i = 0; i < 10; ++i)
            {
                ...
                if (randomNumber < 7)
                {
                    ...
                    gachaList.Add(
                        new(rateUpChId)
                        {
                            ...
                            Stone = isNew
                                ? null
                                : new() { UniqueId = chUniStoneID, StackCount = 50, }
                        }
                    );
                    if (!isNew)
                    {
                ...
                else if (randomNumber < 30)
                {
                    ...
                    if (rateUpIsNormalStudent)
                        poolSize--;

                    var randomPoolIdx = (int)Random.Shared.NextInt64(poolSize);
                    if (normalSSRList[randomPoolIdx].Id == rateUpChId)
                        randomPoolIdx++;
                ...
                else if (randomNumber < 215 || (i == 9 && shouldDoGuaranteedSR))
                {
                    ...
                    gachaList.Add(
                        new(chId)
                        {
                            ...
                            Stone = isNew
                                ? null
                                : new() { UniqueId = chUniStoneID, StackCount = 10, }
                        }
                    );
                    if (!isNew)
                    {
                ...
                else
                {
                    ...
                    gachaList.Add(
                        new(chId)
                        {
                            ...
                            Stone = isNew
                                ? null
                                : new() { UniqueId = chUniStoneID, StackCount = 1, }
                        }
                    );
                    if (!isNew)
                    {
            ...
            try
            {
                ...
                _context.Characters.AddRange(
                    gachaList.Where(x => x.Character != null).Select(x => x.Character)!
                );

                // create if item does not exist
                foreach (var id in itemDict.Keys)
                {
                    var itemExists = _context.Items.Any(x =>
                        x.AccountServerId == account.ServerId && x.UniqueId == id
                    );
                    if (!itemExists)
                    {
                        _context.Items.Add(
                ...
                {
                    _context
                        .Items.Where(x => x.AccountServerId == account.ServerId && x.UniqueId == id)
                        .ExecuteUpdate(setters =>
                            setters.SetProperty(
                                item => item.StackCount,
                                item => item.StackCount + count
                            )
                        );
                }
            ...
            var itemDbList = itemDict
                .Keys.Select(id =>
                    _context
                        .Items.AsNoTracking()
                        .First(x => x.AccountServerId == account.ServerId && x.UniqueId == id)
                )
                .ToList();
            foreach (var gacha in gachaList)
            {
                if (gacha.Stone != null)
                {
                    gacha.Stone.ServerId = itemDbList
                        .First(x => x.UniqueId == gacha.Stone.UniqueId)
                        .ServerId;
                }
            }
