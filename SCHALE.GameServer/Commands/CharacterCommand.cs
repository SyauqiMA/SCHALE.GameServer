...
namespace SCHALE.GameServer.Commands
{
    [CommandHandler(
        "character",
        "Command to interact with user's characters",
        "/character <add|clear> [all|characterId]"
    )]
    internal class CharacterCommand : Command
    {
        public CharacterCommand(IrcConnection connection, string[] args, bool validate = true)
            : base(connection, args, validate) { }

        [Argument(
            0,
            @"^add$|^clear$",
            "The operation selected (add, clear)",
        ...
        [Argument(
            1,
            @"^[0-9]+$|^all$",
            "The target character, value is item id or 'all'",
