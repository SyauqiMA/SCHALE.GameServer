...
namespace SCHALE.GameServer.Commands
{
    [CommandHandler(
        "setaccount",
        "Command to change player's account data (case-sensitive)",
        "/setaccount <|Level|Nickname|RaidSeasonId|Property|...> <Value>"
    )]
    internal class SetAccountCommand : Command
    {
        public SetAccountCommand(IrcConnection connection, string[] args, bool validate = true)
            : base(connection, args, validate) { }

        [Argument(
            0,
            @"^[a-zA-Z]+$",
            "The Account Property you want to change. (case-sensitive)",
        ...
        [Argument(
            1,
            @"",
            "The value you want to change it to, must match the property type.",
            ArgumentFlags.IgnoreCase
        )]
        ...
        public override void Execute()
        {
            PropertyInfo? targetProperty =
                typeof(AccountDB).GetProperty(Property)
                ?? typeof(AccountDB).GetProperty(Property.Capitalize());
                ...
                if (converter != null && converter.CanConvertFrom(typeof(string)))
                {
                    try
                    {
                        ...
                        connection.SendChatMessage(
                            $"Set Player with UID {connection.AccountServerId}'s {Property} to {Value}"
                        );
                    }
                    catch (Exception)
                    {
